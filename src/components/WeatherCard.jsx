    ...
    <div className="relative h-screen w-full md:max-w-[390px] mx-auto bg-[#1E213A] text-white p-6 transition-transform duration-300 ease-in-out transform translate-x-0">
      ...
      <div className='w-full h-2/6 relative '>
        ...
        <figure className='w-48 absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-50'>
          <img src={`/public/${data && data.list && data.list.length > 0 && data.list[0].weather[0].icon}.png`} alt="" />
        </figure>
      </div>

      <div className='mb-14 text-center'>
        <div className=" text-[144px]  leading-none text-center inline-block">
          <span className="text-[#e7e7eb] text-9xl font-Raleway mt-6">{currentTemperature}</span>
      ...
      <div className='text-[#88869D] text-center mb-4'>
        ...
        <div className='flex justify-center mt-10'>
          ...
          <span className='ml-2'>{data && data.city && data.city.name || 'Select a location'}</span>
        </div>
      </div>
    </div>
  )
